...
vector<T> radix_sort(vector<T> &_arr, int _range, Function val)
{
    ...
    for (int i = n - 1; i >= 0; --i)
    {
        output[--counter[val(_arr[i])]] = _arr[i];
    }
    return output;
}
...
int pattern_search(const string &_s, const vi &_sa, const string &pat)
{
    ...
    for (int i = 0; i < pat.size(); ++i)
    {
        ...
        {
            sm = (sl + sr) / 2;
            if (_sa[sm] + i < n && _s[_sa[sm] + i] >= pat[i])
            {
                sr = sm;
            }
            else
...
vi build_lcp(const string &_s, const vi &_sa)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        j = _sa[j + 1];
        while (k < min(n - i, n - j) && _s[i + k] == _s[j + k])
        {
            ++k;
        }
        lcp[inv_sa[i]] = k;
